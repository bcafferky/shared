...
# PreRequisites: You have installed Revolution R Enterprise 7.5.0 or higher on the machine 
...
# back and forth.
# connStr <- "Driver=SQL Server;Server=localhost\\SQL2017;Database=RWalkthru;Uid=bryan;Pwd=bryan;"
...
# ** Reduced rows retrieved and filtered tip_amount for your convenience.  :-)
sampleDataQuery <- "select top 500 tipped, fare_amount, passenger_count,trip_time_in_secs,trip_distance, 
...
# R client to display
# What is notable about the distribution?
start.time <- proc.time()
...
library(ggmap)  
library(mapproj)  
gc <- geocode("1560 Broadway Suite 800. New York, NY 10036", source = "google")  
...
###########################################
# predict and write the prediction results back to 
# a SQL Server table
# Create the object reference to store ther results.
...
#  Note: Script out the procedure to see that it is 
#  a custom written one, not a system one.  
...
#  Bryan simplified query that makes life happy. 
