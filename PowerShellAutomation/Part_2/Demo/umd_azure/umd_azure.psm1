...
function Get-udfConfig  
{
 [CmdletBinding()] param ()
   ...
   # Use $PSScriptRoot to find the config file...
   ...
   foreach ($item in $configdata)  
   {
     ...
     $configobject | Add-Member  -MemberType noteproperty `
...
function Invoke-udfAzureRMLogin   
{
 [CmdletBinding()] param ([string] $azureaccountname)

    $credentialfilepath = Join-Path -Path $PSScriptRoot -ChildPath "azurepw.txt" 
    # $credentialfilepath = "C:\Users\Bryan\Documents\WindowsPowerShell\Modules\umd_azure\azurepw.txt"

    $credin = Get-Content $credentialfilepath | convertto-securestring 
      
    Write-Verbose $credentialfilepath
    
    $credential = New-Object System.Management.Automation.PSCredential (“bryancafferky@outlook.com”,  $credin)
